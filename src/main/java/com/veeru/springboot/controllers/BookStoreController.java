package com.veeru.springboot.controllers;
...
import com.veeru.springboot.exceptions.EmptyInputParam;
import com.veeru.springboot.models.Book;
import com.veeru.springboot.services.BookStoreService;
/**
...
public class BookStoreController {
	...
	public ResponseEntity<?> deleteBook(int bookId) {
		/*
		 * if (bookId > 0) { Book book1 = Book.builder().id(bookId).build(); int
		 * deleteStatus = bookStoreService.deleteBook(book1); if (deleteStatus >= 1) {
		 * return new ResponseEntity<String>("Book deleted succeessfully.",
		 * HttpStatus.OK); } } else { throw new EmptyInputParam("Book id not found",
		 * 400); }
		 */
